...
- [View more](https://hyperskill.org/projects/180) (official project page)
...
**Tip:** If you use Postman or any similar program for testing and receive `403 (Forbidden)`, try to disable **CSRF** (Cross-Site Request Forgery) protection. You can disable this type of protection by calling the methods `.csrf(csrf -> csrf.disable())` on the `HttpSecurity` instance injected in the method where you build a `SecurityFilterChain` object.
...
Further `POST /api/recipe/new` request with basic authentication; `email: Cook_Programmer@somewhere.com`, `password: RecipeInBinary`
```json
{
   "name": "Mint Tea",
   "category": "beverage",
...
Further `PUT /api/recipe/1` request with basic authentication; `email: Cook_Programmer@somewhere.com`, `password: RecipeInBinary`
```json
{
   "name": "Fresh Mint Tea",
   "category": "beverage",
...
Further `GET /api/recipe/1` request with basic authentication; `email: Cook_Programmer@somewhere.com`, `password: RecipeInBinary`
...
Further response for the `GET /api/recipe/1` request with basic authentication; `email: CamelCaseRecipe@somewhere.com`, `password: C00k1es.`
```json
{
   "name": "Fresh Mint Tea",
   "category": "beverage",
...
Further `PUT /api/recipe/1` request with basic authentication; `email: CamelCaseRecipe@somewhere.com`, `password: C00k1es.`
```json
{
   "name": "Warming Ginger Tea",
   "category": "beverage",
...
Further `DELETE /api/recipe/1` request with basic authentication; `email: CamelCaseRecipe@somewhere.com`, `password: C00k1es.`
